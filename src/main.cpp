...
void create_second_screen_tab2(lv_obj_t * parent, const std::string& key) {
    ...
    if (!book.isNull()) {
        String titulo = book["titulo"].as<String>();
        String autor = book["autor"].as<String>();
        int paginas_total = book["paginas_total"].as<int>();
        int pagina_actual = book["pagina_actual"].as<int>();
...
static void tab4_content(lv_obj_t * parent) {
    ...
    // Get the book data from Firebase
    std::vector<JsonObject> books = get_book_data(); // Get all books
    ...
    // Iterate through each book
    for(auto& book : books) {
        int current_pages = book["pagina_actual"].as<int>();
        int total_pages_book = book["paginas_total"].as<int>();
        String title = book["titulo"].as<String>();
...
void create_second_screen_tab4(lv_obj_t *padre) {
    ...
    // Set the tick labels for the X axis
    lv_chart_set_axis_tick(chart, LV_CHART_AXIS_PRIMARY_X, 5, 5, books.size(), 1, true, 50); // Set the tick count to the number of books

    // Set the point count to the number of books
    lv_chart_set_point_count(chart, books.size());
    ...
    // For each book, calculate the percentage of pages read and add a series to the chart
    for(auto& book : books) {
        String book_key = book["key"].as<String>(); // Get the book key
        book_keys.push_back(book_key.c_str()); // Store the book key for later use
        JsonObject libro = book;
